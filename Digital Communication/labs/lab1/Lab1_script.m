...
% there is another arguement fs: The bit flipping probability and has a
% default value 1, we can change it to 0.5 if the receiver gives 0s&1s with
% probability 0.5 
...
% minimum values 
min_case_1 = min(BER_case_1_vec);
min_case_2 = min(BER_case_2_vec);
min_case_3 = min(BER_case_3_vec);
...
%%% WRITE YOUR CODE HERE
fs_vect = 0:1:10;
BER_case_2_rep_vec = zeros(size(fs_vect));

for fs_ind = 1:length(fs_vect)
    sample_seq = GenerateSamples(bit_seq,fs_vect(fs_ind));
    rec_sample_seq = BSC(sample_seq,fs_vect(fs_ind),p);
    rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_2', fs_vect(fs_ind));
    BER_case_2_rep_vec(fs_ind) = ComputeBER(bit_seq,rec_bit_seq);
end

% Plotting results

figure
plot(fs_vect,BER_case_2_rep_vec,'x-b','linewidth',2); 
